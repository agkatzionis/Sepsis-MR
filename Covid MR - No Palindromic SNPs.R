...
## Detect and remove palindromic SNPs.
covid.palindromic <- which(covid$effect_allele == "A" & covid$other_allele == "T" |
                           covid$effect_allele == "T" & covid$other_allele == "A" |
                           covid$effect_allele == "G" & covid$other_allele == "C" |
                           covid$effect_allele == "C" & covid$other_allele == "G" )
length(covid.palindromic)
covid <- covid[- covid.palindromic, ]
dim(covid)
...
## Subset G-X associations to exclude any SNPs not included in the covid dataset.
## Since we have already discarded palindromic SNPs from the covid dataset, 
## this will get rid of the palindromic SNPs in the other datasets.
...
## Do the plot.
tiff(file = "Covid_IVW_NoPal.tiff", width = 1500, height = 1000, pointsize = 11, res = 300, compression = 'lzw')
par(mar = c(4, 3.5, 3, 7))
...
## Start plotting.
tiff(file = "Covid_All_Methods_NoPal.tiff", width = 2000, height = 1600, pointsize = 10, res = 300, compression = 'lzw')
...
## Create the Figure.
tiff(file = "Covid_MVMR_Lipids_NoPal.tiff", width = 1500, height = 800, pointsize = 11, res = 300, compression = 'lzw')
